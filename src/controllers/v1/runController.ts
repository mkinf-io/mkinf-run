...
import RunSessionsRepository from "../../repositories/RunSessionsRepository";
...
// POST /:owner/:repo
// POST /:owner/:repo@:version
export const startRunSession = async (req: Request, res: Response) => {
	try {
		if (!req.db) { return res.status(500).json({ status: 500, message: "Server error" }); }
		if (!req.keyId) { return res.status(400).json({ status: 400, message: "Missing key ID" }); }
		// Get the latest release for the repository
		...
		const runLog = {
			...
			build_number: latestRelease.build_number,
			sandbox_id: transport.sandboxId,
			pid: transport.pid,
			price_run_second: latestRelease.price_run_second,
		}
		// Detach connection
		await transport.detach();
		// Log on db
		const logRes = await new RunSessionsRepository(req.db).create(runLog);
		if (logRes.error) {
			console.log("Error while logging run:\n", logRes.error);
			console.log("Complete log:\n", runLog);
			client.close();
			await transport.close();
			return res.status(500).json({ status: 500, message: "Error while creating run session" });
		}
		return res.status(200).json({ status: 200, data: { session_id: logRes.data?.id }, duration });
	} catch (error) {
		console.log("Error", error);
		res.status(500).json({ status: 500, message: "Server error" });
	}
...
export const listTools = async (req: Request, res: Response) => {
	try {
		...
		if (!req.keyId) { return res.status(400).json({ status: 400, message: "Missing key ID" }); }
		// Get session
		const sessionRes = await new RunSessionsRepository(req.db).get({ key_id: req.keyId, id: req.params.sessionId });
		console.log("session", sessionRes);
		if (sessionRes.error || !sessionRes.data) { return res.status(404).json({ status: 404, message: "Session not found" }); }
		const session = sessionRes.data;
		// Get the latest release for the repository
		// TODO: Check version
		const latestReleaseRes = await new HostedReleasesRepository(req.db).getLatest({
			key_id: req.keyId,
			owner: session.organization_name,
		...
		await transport.detach();
		// Log on db
		const logRes = await new RunsRepository(req.db).create(runLog);
		if (logRes.error) {
			console.log("Error while logging run:\n", logRes.error);
			console.log("Complete log:\n", runLog);
		}
...
export const runAction = async (req: Request, res: Response) => {
	try {
		if (!req.db) { return res.status(500).json({ status: 500, message: "Server error" }); }
		if (!req.keyId) { return res.status(400).json({ status: 400, message: "Missing key ID" }); }
		// Get session
...
export const closeRunSession = async (req: Request, res: Response) => {
	// try {
	// 	if (!req.db) { return res.status(500).json({ status: 500, message: "Server error" }); }
	// 	if (!req.keyId) { return res.status(400).json({ status: 400, message: "Missing key ID" }); }
	// 	// Get session
